...
class ContentDistributionEventHandlerTest {
  ...
  @InjectMocks ContentDistributionEventHandler contentDistributionEventHandler;
  ...
  void setUp() throws LoginException {
    ...
    when(indexingService.getGeoSiteRoot(path, resourceResolver))
        .thenReturn("/content/kpmgpublic/fr/fr/");
  ...
  void testHandleEventForLeader() {
    Event event = distributionEvent.toEvent(Constants.ELASTIC_SEARCH_JOBS_QUEUE);

    when(instanceDescription.isLeader()).thenReturn(true);
    when(jobManager.addJob(anyString(), anyMap())).thenReturn(job);
    contentDistributionEventHandler.handleEvent(event);
    ...
    verify(jobManager, atLeastOnce()).addJob(Constants.ELASTIC_SEARCH_JOBS_QUEUE, jobProperties);
  }
