...
public class DisclaimerDataSourceProvider implements CaConfigDatasourceProvider {

  static final String NAME = "disclaimer";
  ...
  public List<DataSourceOption> getOptions(ConfigurationBuilder configurationBuilder) {
    return Optional.ofNullable(configurationBuilder)
        .map(b -> b.as(DisclaimerConfig.class))
        .map(DisclaimerConfig::items)
