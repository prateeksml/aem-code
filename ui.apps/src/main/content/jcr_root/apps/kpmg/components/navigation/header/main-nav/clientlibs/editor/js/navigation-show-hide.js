/**
 * NOTE: this is a copy of the OOTB show hide script, adjusted for mainnavigation to avoid submitting hidden field values.
 * 
 ...
 * How to use:
 *
 * - add the class kpmg-mainnavigation-dropdown-showhide to the dropdown/select element
 * - add the data attribute kpmg-mainnavigation-dropdown-showhide-target to the dropdown/select element, value should be the
...
(function (document, $) {
    ...
    $(document).on("foundation-contentloaded", function (e) {
        ...
        showHideHandler($(".kpmg-mainnavigation-dropdown-showhide", e.target));
    });

    $(document).on("selected", ".kpmg-mainnavigation-dropdown-showhide", function (e) {
    ...
    function showHide(component, element) {
        ...
        var target = $(element).data("kpmgMainnavigationDropdownShowhideTarget");
        var $target = $(target);
     ...
     */
     function setVisibilityAndHandleFieldValidation($element, show) {
         ...
         disableChildSubmittables($element, !show);
     }

     /**
      * Disables all submittable form elemets inside the provided element.
      * @param {jQuery} $element 
      * @param {Boolean} disable 
      * @returns 
      */
     function disableChildSubmittables($element, disable) {
            var form = $element.closest("form");
            if (!form.length) return;
            form = form[0];
            Array.from(form.elements).forEach(function (formElement) {
                // check if the element is a child of the element we want to disable
                if ($element.has(formElement).length) {
                    if (disable) {
